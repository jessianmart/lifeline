...
class IsolatedSandboxExecutor:
    ...
    async def execute_physical_command(
        ...
        If bypass_sandbox=True, executes the command directly on the host OS for maximum performance (10ms).
        """
        ...
        # Determine if we should bypass the Docker sandbox
        use_docker = self._check_docker_available() and not bypass_sandbox
        ...
        if use_docker:
            ...
            try:
                ...
                cmd_str = shlex.join(containerized_args)
                tick_cmd = f"mkdir -p /tmp/{tick_id} && find /usr/src/app -mindepth 1 -maxdepth 1 -not -name '.git' -not -name '__pycache__' -not -name '*.db*' -exec cp -rp {{}} /tmp/{tick_id}/ \\; && cd /tmp/{tick_id} && {cmd_str}"
            ...
            # Docker unavailable or bypass_sandbox requested: Evaluate safety permissions
            if not bypass_sandbox and not self.unsafe_allow_host_execution:
            ...
            # Print warning only when bypassing Docker under unsafe fallback mode (not explicit bypass)
        ...
        try:
            ...
            process = subprocess.run(
                ...
                capture_output=True,
                text=True,
                cwd=execution_dir if not use_docker else None, # CWD is inside container's -w
                timeout=30
            )
            ...
            result = {
                ...
                "stdout": stdout_clean,
                "stderr": stderr_clean,
                "isolated_via": "DockerContainer" if use_docker else "HostSubprocess"
            }
