...
from lifeline.context.memory import BaseVectorMemory
...
class FederatedContextRetrieval:
    ...
    def __init__(
        ...
        vector_memory: Optional[BaseVectorMemory] = None
    ):
        self.store = store
    ...
    async def retrieve_raw_context(
        self, 
        workflow_id: str, 
        ...
        # 5. Fetch semantic memory records if query vector and store are available
        semantic_memory = []
        if query_vector and self.vector_memory:
            results = await self.vector_memory.search_related_episodes(
                query_vector, 
                limit=5, 
                filter_workflow_id=workflow_id
            )
            semantic_memory = [r.model_dump() for r in results]
